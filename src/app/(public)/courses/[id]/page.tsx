    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[70vh] gap-4">
                ...
                <p className="font-bold text-slate-500">Retrieving course details...</p>
            </div>
        );
    }
    ...
    if (!course) {
        return (
            <div className="text-center py-20 px-6 max-w-lg mx-auto">
                <h1 className="text-3xl font-black text-slate-900 mb-4">Course Not Found</h1>
                <p className="text-slate-500 mb-8">The course you are looking for might have been moved or unpublished.</p>
                <Link href="/courses">
                    <Button className="rounded-2xl h-14 px-10 font-bold">Browse All Courses</Button>
                </Link>
            </div>
        );
    }
        ...
        <div className="pb-32 bg-white">
                            ...
                            <div className="flex flex-wrap items-center gap-8 py-6 border-y border-slate-200">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 rounded-full bg-slate-200 overflow-hidden border-2 border-white shadow-md">
                                        <img src={course.instructorIds?.[0]?.image || "https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=1974"} alt="Instructor" className="w-full h-full object-cover" />
                                    </div>
                                    <div>
                        ...
                        <div className="flex-1 w-full lg:sticky lg:top-32 animate-in fade-in slide-in-from-right-4 duration-700">
                            <div className="bg-white rounded-[3rem] border-8 border-slate-50 shadow-2xl p-8 space-y-8 overflow-hidden relative group">
                                <div className="aspect-video rounded-3xl bg-slate-900 flex items-center justify-center overflow-hidden relative">
                                    <img src={course.thumbnail} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700" alt="thumbnail" />
            ...
            <div className="container mx-auto px-4 mt-20">
                <div className="flex flex-col lg:flex-row gap-20">
                                ...
                                <div className="space-y-4">
                                    {lessons.map((lesson, idx) => (
                                        <div key={idx} className="bg-white rounded-3xl p-6 border border-slate-100 flex items-center justify-between group hover:border-primary/30 transition-all hover:shadow-xl hover:shadow-slate-100">
                                            ...
                                            {lesson.isFree ? (
                                                <Badge className="bg-emerald-50 text-emerald-600 border-none font-black text-[9px] uppercase tracking-widest px-3">Free Preview</Badge>
                                            ) : (
                                 ...
                                 <div className="space-y-12">
                                     {course.instructorIds?.map((inst: any) => (
                                         <div key={inst._id} className="flex flex-col md:flex-row gap-8 items-center md:items-start">
                                             <div className="w-32 h-32 rounded-[2.5rem] bg-slate-200 overflow-hidden shrink-0 shadow-2xl border-4 border-white">
                                                 <img src={inst.image || "https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=1974"} className="w-full h-full object-cover" alt="inst" />
                                             </div>
                    ...
                    {/* Left out sidebar features for simplicity or could be added back */}
                </div>
            </div>
        </div>
    );
...
function Badge({ children, className }: { children: React.ReactNode, className?: string }) {
    return (
        <span className={`px-2 py-1 rounded text-xs inline-flex items-center ${className}`}>
            {children}
        </span>
    );
}
