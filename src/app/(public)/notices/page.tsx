...
export default async function NoticesPublicPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 relative pt-24 pb-20 overflow-hidden">
            {/* Ambient Backgrounds */}
            ...
            <div className="container px-6 mx-auto relative z-10 max-w-5xl">
                {/* Header Section */}
                <div className="text-center md:text-left mb-16 max-w-3xl space-y-6">
                    <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-blue-50 text-blue-600 font-bold border border-blue-100 shadow-sm text-xs tracking-widest uppercase">
                        <Bell className="w-4 h-4" /> Live Broadcast
                    </div>
                    <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight leading-none bg-clip-text text-transparent bg-gradient-to-br from-slate-900 to-slate-600">
                ...
                {notices.length === 0 ? (
                    ...
                    <div className="space-y-8 relative">
                        ...
                        {notices.map((notice: any, index: number) => {
                            ...
                            return (
                                <div key={notice._id} className="relative flex flex-col md:flex-row items-start gap-6 md:gap-10 group animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${index * 100}ms` }}>
                                    
                                    {/* Date Bubble */}
                                    <div className="flex-shrink-0 flex items-center gap-4 w-full md:w-auto">
                                        <div className="relative z-10 w-20 h-20 md:w-28 md:h-28 bg-white rounded-3xl md:rounded-[2rem] flex flex-col items-center justify-center border-2 border-slate-100 shadow-lg shadow-slate-200/50 group-hover:border-blue-500 group-hover:shadow-blue-500/20 transition-all duration-300 group-hover:-translate-y-1">
                                            <span className="text-xs font-black text-slate-400 tracking-widest uppercase mb-1">{month}</span>
                                            <span className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter leading-none">{day}</span>
                                            <span className="text-[10px] font-bold text-slate-400 tracking-wider mt-1">{dateObj.getFullYear()}</span>
                                        </div>
                                    </div>

                                    {/* Content Card */}
                                    <div className="flex-1 bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden group-hover:-translate-y-1 transition-transform duration-300">
