        ...
        <div className="relative h-[85vh] md:h-[90vh] flex items-center justify-center overflow-hidden bg-slate-950">
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-900/40 to-slate-950 z-[5]" />
                    
                    {/* Radial Glows */}
                    <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-primary/20 rounded-full blur-[120px] mix-blend-screen animate-pulse duration-[4000ms]" />
                    <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-indigo-500/20 rounded-full blur-[120px] mix-blend-screen animate-pulse duration-[6000ms]" style={{ animationDelay: '2s' }} />
                    ...
                    {content.image && (
                        <Image 
                            ...
                            fill 
                            className="object-cover opacity-60 scale-105 motion-safe:animate-[zoom_15s_infinite_alternate]" 
                            priority 
                        />
                    )}
                </motion.div>
            ...
            {slides.length > 1 && (
                <>
                    <button 
                        onClick={prevSlide}
                        className="absolute left-6 top-1/2 -translate-y-1/2 z-30 w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-all backdrop-blur-md"
                    >
                        <ChevronLeft className="w-8 h-8" />
                    </button>
                    <button 
                        onClick={nextSlide}
                        className="absolute right-6 top-1/2 -translate-y-1/2 z-30 w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-all backdrop-blur-md"
                    >
                    ...
                    <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-30 flex gap-3">
                        {slides.map((_, i) => (
                            <button
                                ...
                                className={`transition-all duration-500 rounded-full h-2 ${current === i ? "w-12 bg-primary shadow-[0_0_15px_rgba(37,99,235,1)]" : "w-3 bg-white/30"}`}
                            />
                        ))}
                    </div>
                </>
